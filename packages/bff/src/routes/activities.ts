...
activitiesRouter.post('/', (req, res) => {
  const { categoryId, categoryName, categoryColor, startTime, endTime, productivityLevel, note, title } = req.body;
  ...
  if (![1, 2, 3, 4].includes(prodLevel)) {
    return res.status(400).json({ error: 'Productivity level must be 1, 2, 3 or 4' });
  }
  ...
  if (Number.isNaN(parsedStartTime.getTime()) || Number.isNaN(parsedEndTime.getTime())) {
    return res.status(400).json({ error: 'Start and end times must be valid ISO dates' });
  }
