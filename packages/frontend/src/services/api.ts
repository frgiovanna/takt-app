...
export const api = {
  ...
  getCategories: async (): Promise<Category[]> => {
    try {
      ...
      console.warn('Failed to fetch categories from BFF, using local storage fallback:', err);
      const local = localStorage.getItem('takt_categories');
      if (local) return JSON.parse(local);

      const defaults: Category[] = [
        { id: 'cat-global-1', name: 'Reuniões', color: '#38bdf8', isCustom: false },
        { id: 'cat-global-2', name: 'Programação', color: '#10b981', isCustom: false },
        { id: 'cat-global-3', name: 'Estudo', color: '#a855f7', isCustom: false },
        { id: 'cat-global-4', name: 'Planejamento', color: '#f59e0b', isCustom: false },
      ];
  ...
  createCategory: async (name: string, color: string): Promise<Category> => {
    try {
      ...
      if (name.length > 50) throw new Error('Category name must be maximum 50 characters');
  ...
  getActivities: async (): Promise<Activity[]> => {
    try {
      ...
      console.warn('Failed to fetch activities from BFF, using local storage fallback:', err);
